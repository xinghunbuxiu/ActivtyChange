...
private constructor(context: Context, private val mParentView: ViewGroup, private val mCallback: Callback) {
    ...
    private lateinit var mInitialEdgeTouched: IntArray
    ...
    fun setSensitivity(context: Context, sensitivity: Float) {
        val s = max(0f, Math.min(1.0f, sensitivity))
    ...
    private fun computeSettleDuration(child: View, dx: Int, dy: Int, xvel: Int, yvel: Int): Int {
        var xvel = clampMag(xvel, minVelocity.toInt(), mMaxVelocity.toInt())
        var yvel = clampMag(yvel, minVelocity.toInt(), mMaxVelocity.toInt())
        val absDx = abs(dx)
        val absDy = abs(dy)
        ...
        val xduration = mCallback?.getViewHorizontalDragRange(child)?.let { computeAxisDuration(dx, xvel, it) }
        val yduration = mCallback?.getViewVerticalDragRange(child)?.let { computeAxisDuration(dy, yvel, it) }
    ...
    private fun computeAxisDuration(delta: Int, velocity: Int, motionRange: Int): Int {
        ...
        val distanceRatio = min(1f, abs(delta).toFloat().div(width!!))
        val distance = halfWidth?.plus(halfWidth * distanceInfluenceForSnapDuration(distanceRatio))

        val duration: Int
        val velocity = abs(velocity)
        duration = when {
            velocity > 0 -> 4 * round(1000 * abs(distance?.div(velocity)!!))
            else -> {
    ...
    private fun saveInitialMotion(x: Float, y: Float, pointerId: Int) {
        ...
        mInitialEdgeTouched?.set(pointerId, getEdgeTouched(x.toInt(), y.toInt()))
    ...
    fun setDragState(state: Int) {
        if (viewDragState != state) {
            ...
            if (state == STATE_IDLE) capturedView = null
        }
    }
    ...
    private fun getEdgeTouched(x: Int, y: Int): Int {
        ...
        if (x < mParentView.left.plus(edgeSize))
            result = EDGE_LEFT
        if (y < mParentView.top.plus(edgeSize))
            result = EDGE_TOP
        if (x > mParentView.right.minus(edgeSize))
            result = EDGE_RIGHT
        if (y > mParentView.bottom.minus(edgeSize))
            result = EDGE_BOTTOM
