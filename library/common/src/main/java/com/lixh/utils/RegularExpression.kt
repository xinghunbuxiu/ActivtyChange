...
/**
 *
 * @Title: RegularExpression.java
 * @Package com.cihon.util
 * @Description: 正则工具类
 * @author chang
 * @date 2015年6月11日 下午4:37:43
 * @version V1.0
 */
object RegularExpression {
    ...
    /**
     * 验证Email
     * @param email email地址，格式：zhangsan.qi@sina.com，zhangsan@xxx.com.cn，xxx代表邮件服务商
     * @return 验证成功返回true，验证失败返回false
     */
    fun checkEmail2(email: String): Boolean {
    ...
    /**
     * 验证车牌号码   大小写 不区分"^[\u4e00-\u9fa5]{1}[a-zA-Z]{1}[a-zA-Z_0-9]{5}$"
     * @param "验证车牌号码   只能大小
     * @return 验证成功返回true，验证失败返回false
     */
    fun checkLicensePlateNo(number: String): Boolean {
    ...
    fun checkIdCard(idCard: String): Boolean {
        val regex = "[1-9]\\d{16}[a-zA-Z0-9]{1}"
    ...
    /**
     * 六位数字匹配
     * @param postcode 六位数字匹配
     * @return 验证成功返回true，验证失败返回false
     */
    fun checkSix(postcode: String): Boolean {
        val regex = "[1-9]\\d{5}"
        return Pattern.matches(regex, postcode)
    }
    ...
    /**
     * 八位数字匹配
     * @param postcode 八位数字匹配
     * @return 验证成功返回true，验证失败返回false
     */
    fun checkEight(postcode: String): Boolean {
        val regex = "[1-9]\\d{7}"
        return Pattern.matches(regex, postcode)
    }
    ...
    /**
     * 八位数字匹配
     * @param postcode 五位到九位数字
     * @return 验证成功返回true，验证失败返回false
     */
    fun checkFiveNine(postcode: String): Boolean {
        val regex = "^[0-9]{5,9}$"
        return Pattern.matches(regex, postcode)
    }
