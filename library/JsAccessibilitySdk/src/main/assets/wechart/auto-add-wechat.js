...
function onLauncherUi() {
	sleep(500);
	request.log('点击搜索');
	var node = request.findViewByNodeIndex("com.tencent.mm:id/jf", 1);
...
function add_friend(name) {
	...
	var node = request.findViewByID("com.tencent.mm:id/kh");
	request.log('开输入---------------------------------------->' + name);
	request.inputText(node, name);
	sleep(500);
	request.log('点击添加---------------------------------------->');
	request.clickTextViewByID("com.tencent.mm:id/bxc")
}
//添加到通讯录
function onContactInfoUI() {
	...
	if (node != null) {
		...
		request.log('添加到通讯录---------------------------------------->');
		request.clickTextViewByID("com.tencent.mm:id/cs");
	}
...
function SayHiWithSnsPermissionUI() {
	...
	var node = request.findViewByID("com.tencent.mm:id/e0o");
	request.log('填充发送内容---------------------------------------->' + sendText);
	request.inputText(node, sendText);
	sleep(500);
	request.log('点击发送---------------------------------------->');
	request.clickTextViewByID("com.tencent.mm:id/jx")
	sleep(500);
